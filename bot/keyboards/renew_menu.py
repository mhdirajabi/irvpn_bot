...
def get_renew_menu():
    ...
    for i in range(0, len(categories), 2):
        ...
        for category in categories[i:i+2]:
            text = (
                "📈 ┖ اکانت حجمی" if category == "volume" else
                "♾️ اکانت نامحدود" if category == "unlimited" else
                "🪗 اکانت تست"
            )
            row.append(InlineKeyboardButton(text=text, callback_data=f"renew_{category}"))
        keyboard.append(row)
    # اضافه کردن دکمه بازگشت
    keyboard.append([InlineKeyboardButton(text="⬅️ بازگشت به منوی اصلی", callback_data="back_to_main")])
...
def get_renew_plan_menu(category: str):
    ...
    for i in range(0, len(plans), 2):
        ...
        for plan in plans[i:i+2]:
            ...
            row.append(InlineKeyboardButton(text=text, callback_data=f"renewselect_{plan['id']}"))
        keyboard.append(row)
    # اضافه کردن دکمه‌های بازگشت
    keyboard.append([InlineKeyboardButton(text="⬅️ بازگشت", callback_data="buy_back")])
    keyboard.append([InlineKeyboardButton(text="⬅️ منوی اصلی", callback_data="back_to_main")])
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
